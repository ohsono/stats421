#!/usr/bin/env python3
"""
Investment Scoring Methodology V2 - ZORI Enhanced
...
class ZScoreV2Scoring:
    ...
    def run(self):
        print("="*100)
        print("🚀 STARTING Z-SCORE V2 ANALYSIS (WITH ZORI RENT DATA)")
        print("="*100)
        ...
        # Weights: Growth 40%, Income 30%, Affordability 20%, Demo 10%
        df['z_score_v2'] = (
            df['z_growth'] * 0.40 +
            df['z_income'] * 0.30 +
            df['z_affordability'] * 0.20 +
            df['z_demographics'] * 0.10
        )
        
        # Normalize to 0-100
        df['score_v2_norm'] = ((df['z_score_v2'] + 3) / 6) * 100
        df['score_v2_norm'] = df['score_v2_norm'].clip(0, 100)
        
        # Sort
        df = df.sort_values('z_score_v2', ascending=False).reset_index(drop=True)
        
        # 6. Display Results
        print("\n" + "="*120)
        print("🏆 TOP 20 COUNTIES - V2 MODEL (Growth + Real Income)")
        print("="*120)
        print(f"{'Rank':<5} {'County':<25} {'V2 Score':<10} {'Yield':<8} {'Rent':<10} {'Price':<12} {'Growth':<8}")
        print("-" * 120)
        
        for idx, row in df.head(20).iterrows():
            est_flag = "*" if row['is_estimated_rent'] else " "
            print(f"{idx+1:<5} {row['county']:<25} {row['score_v2_norm']:>6.1f}     {row['actual_yield']:>5.2f}%{est_flag}  "
        ...
        # Save
        output_file = self.processed_dir / 'all_counties_ranking_v2.csv'
        df.to_csv(output_file, index=False)
        print(f"\n✅ Saved V2 rankings to: {output_file}")
        
        return df

if __name__ == "__main__":
    scorer = ZScoreV2Scoring()
    scorer.run()
